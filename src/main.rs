...
use raytracing_in_a_weekend_rust::{hittable_list, sphere, vector, color, point, lambertian, hittable, metal, dielectric};
use std::env;
use std::fs::File;
use std::io;
use std::rc::Rc;
...
fn main() -> io::Result<()> {
    ...
    let material_left = Rc::new(dielectric::Dielectric::new(1.5));
    let material_bubble = Rc::new(dielectric::Dielectric::new(1.0 / 1.5));
    let material_right = Rc::new(metal::Metal::new(color::Color::new(0.8, 0.6, 0.2), 0.0));
    let world = hittable_list::HittableList::new(vec![
        ...
        Rc::new(sphere::Sphere::new(point::Point3::new(0.0, 0.0, -1.2), 0.5, material_center.clone())) as Rc<dyn hittable::Hittable>,
        Rc::new(sphere::Sphere::new(point::Point3::new(-1.0, 0.0, -1.0), 0.5, material_left.clone())) as Rc<dyn hittable::Hittable>,
        Rc::new(sphere::Sphere::new(point::Point3::new(-1.0, 0.0, -1.0), 0.4, material_bubble.clone())) as Rc<dyn hittable::Hittable>,
